<template>
    <div :class="{ dropdown_opened: isMenuOpen }" class="dropdown">
        <button
            :class="{ dropdown__toggle_icon: hasSomeoneIcon }"
        ...
        <div v-show="isMenuOpen" class="dropdown__menu" role="listbox">
            <button
                ...
                :key="option.value"
                :class="{ dropdown__item_icon: hasSomeoneIcon }"
        ...
        <select v-show="false" v-model="modelProxy">
...
const hasSomeoneIcon = computed(() => props.options.some((item) => item.icon));
